...
func (l *LMSConfig) ReadAllBorrowedBooksCSV(w http.ResponseWriter, r *http.Request) {
	...
	for _, t := range BorrowedBooksData {
		row := []string{
	...
	filename := fmt.Sprintf("borrowed_books_%s.csv", today)
